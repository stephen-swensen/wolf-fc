...
| `cutscenes.fc` | `bj_victory`, `death_cam` (with nested `sub`), `intermission`, `episode_end`, `endart_screen`, `pg13_screen`, `game_over`, `title_screen` | Per-phase state machines + renderers. `intermission` owns the scoring constants + `par_times[]` table + `compute_*` helpers. `endart_screen` parses the OG article-markup syntax (`^P` page / `^E` end / `^Cnn` color / `^Bx,y,w,h` fill / `^Lyyy,xxx` jump / `^>` tab / `^Gy,x,p` graphic / `^Tx,y,p,t` timed / `^;` comment) and word-wraps text inside the four-piece help-window border. `pg13_screen` runs the parental-advisory splash. |
