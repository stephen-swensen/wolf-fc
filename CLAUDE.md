...
| `level.fc` | `difficulty`, `scoring`, `tilemap`, `areas`, `spawn`, `doors`, `pushwall`, `pickups` | Level geometry + enemy/sprite spawning + door / pushwall animation + pickup collection. `difficulty` holds the 4 picker constants. `scoring` is a leaf module with `scoring.add(g, amount)` — the one chokepoint every score-awarding feature (pickups, kills, intermission bonus) routes through so the IDKFA score-lock works uniformly. |
