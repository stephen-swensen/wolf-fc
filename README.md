...
<p align="center">
  <a href="screenshots/ss_001-display.png"><img src="screenshots/ss_001-display.png" width="24%" alt="Screenshot 1"></a>
  <a href="screenshots/ss_002-display.png"><img src="screenshots/ss_002-display.png" width="24%" alt="Screenshot 2"></a>
  <a href="screenshots/ss_003-display.png"><img src="screenshots/ss_003-display.png" width="24%" alt="Screenshot 3"></a>
  <a href="screenshots/ss_004-display.png"><img src="screenshots/ss_004-display.png" width="24%" alt="Screenshot 4"></a>
</p>
...
| `--debug` | Start with the debug overlay on (also toggleable in-game with **TAB + D**). Draws a small top-right readout: **FPS** + per-frame *work* vs. the vsync budget (`work/budget ms`, red while work overruns the budget or just after a dropped frame); a cumulative **drops** count of genuinely missed vblanks plus **jit**, the EMA sub-frame timing wobble in ms (scheduler / compositor wake latency — healthy variance, not lost frames), both measured with the high-res timer so they catch stutter the millisecond clock can't; and the active framebuffer / screen resolution + AA mode. Drawn straight to the display, so it never lands in screenshots. |
...
**Frame pacing.** The simulation is variable-`dt` — `tick(w, dt)` advances everything by the real elapsed time — and under vsync the display cadence is rock-steady, but the *measured* `dt` is not. It comes from millisecond-resolution `SDL_GetTicks`, so a true 16.667 ms frame reads as 16 or 17, and scheduler / compositor wake jitter nudges it a little further. Feeding that noisy `dt` straight into `position += velocity · dt` makes each displayed frame advance the world by a slightly different amount even though the frames are shown at a constant rate — sub-pixel motion judder, most visible on a slow pan along a wall. So the loop snaps `dt` to the nearest whole multiple of the display's refresh interval (`refresh_interval_ms`, read from the active display mode at startup, re-derived on cross-monitor moves, and printed in the `Display: … Hz` banner) — but only when the frame already landed within 20 % of a clean multiple. A steady frame → exactly one interval; a genuinely dropped frame (~2× interval) → a clean 2× catch-up step; a frame that's nowhere near a multiple (vsync not honored, free-running on battery) → left raw, since rounding it would run the sim fast or slow. The snap disables itself when the refresh rate is unknown, and never runs in `--test` mode (which uses a fixed `dt = 1/35` and bypasses the loop), so the regression suite stays bit-stable.

This is deliberately the *whole* of the frame-pacing story. Because the snap locks one displayed frame to exactly one correctly-sized sim step, motion is already as smooth as the display allows — there is nothing left to interpolate *between*. The textbook "mature" extension — a fixed-timestep accumulator with render interpolation — is intentionally **not** built here: in a variable-`dt` engine it would only buy determinism across refresh rates (not a project goal) while adding up to one sim-step of input latency. Genuine dropped frames (work over budget) are the only residual judder, and no pacing trick can display a frame that was never rendered; the snap just makes the catch-up clean. On a fast machine comfortably within budget the artifact the snap removes is small (~±1 ms of `dt` wobble, often below the perception threshold) — its real payoff is on marginal or busy systems.
